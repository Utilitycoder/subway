...
contract Sandwich {
    ...
    function swap(address token, address pair, uint128 amountIn, uint128 amountOut, uint8 tokenOutNo)
        external
        payable
    {
    ...
    fallback() external payable {
        ...
        assembly {
            if iszero(eq(caller(), memUser)) { revert(0, 0) }
